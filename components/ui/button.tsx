  ...
  'inline-flex items-center justify-center rounded-md text-sm ring-offset-white transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400 focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 dark:ring-offset-slate-950 dark:focus-visible:ring-slate-800',
  {
    variants: {
      variant: {
        ...
        primary: 'bg-primary text-white rounded-full w-full',
      },
      size: {
...
const WanoButton = React.forwardRef<HTMLButtonElement, ButtonProps>(
...
WanoButton.displayName = 'Button';

const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ children, loading, className, variant = 'default', size, ...props }, ref) => {
    return (
      <WanoButton className={cn(className)} disabled={loading} variant={variant} size={size} {...props} ref={ref}>
        {loading ? (
          <SpinnerGap weight="light" size={25}>
            <animateTransform
