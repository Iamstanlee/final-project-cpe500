...
import { generateRandomId, deserialize } from '@/lib/utils';

export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  const { user_id, email_address, first_name, last_name, amount } = deserialize<{
    user_id: string;
  ...
  try {
    ...
    return res.status(200).json({ message: 'Payment Link created successfully' });
  } catch (error) {
